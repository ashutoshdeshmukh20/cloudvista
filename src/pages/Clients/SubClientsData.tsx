    ...
    return (
        <div className="page-content">
            <Container fluid={true}>
                <Breadcrumbs title="Clients" breadcrumbItem="subClient Details" />
                <Card>
                    <CardBody> 
                        <div className="d-flex justify-content-between mb-3">
                            <h1 className="card-title">{orgName} :</h1>
                            <Button color="primary" onClick={handleViewClients}>View Subclients</Button>
                        </div>
                                            ...
                                            <div className="d-flex flex-column mt-3">
                                                {client.subClients.map((client) => (
                                                    <Form key={client.id}>
                                                        ...
                                                        <FormGroup >
                                                            <Label for={`email${client.id}`}>Contact No.</Label>
                                                            <Input type="text" name={`email${client.id}`} id={`email${client.id}`} defaultValue={client.mobile} readOnly={!editable} />
                                                        </FormGroup>
                                                        <FormGroup >
                                                            <Label for={`subDes${client.id}`}>Designation</Label>
                                                            <Input type="text" name={`subDes${client.id}`} id={`subDes${client.id}`} defaultValue={client.subDesignation} readOnly={!editable} />
                                                        </FormGroup>
                                                        <div className="d-flex justify-content-end">
                                                            <Button color="danger" type="button" onClick={() => setEditable(false)}>Cancel</Button>
                                                            <Button color='primary' type='button' className='mx-2' onClick={toggleEditable}>{editable ? 'Save' : 'Edit'}</Button>
                                    ...
                                    {orgClients.map(client => (
                                        <Col md="5" key={client.id}>
                                            <Form>
                                                <FormGroup>
                                                    <div className="d-flex flex-column mt-3">
                                                        {client.subOrgs.map((service, index) => (
